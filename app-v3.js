...
function renderDashboard(){
  ...
  app.innerHTML=pageHead('Панель управления','Главные показатели, сроки и текущие задачи проекта.')+`
    <div class="v3-dashboard-hero">
      ...
      <aside class="v3-next-card"><h3>Ближайший рабочий срок</h3>${next?`<span class="v3-next-date">${formatDate(next.due_date)} · ${daysText(next.due_date)}</span><strong>${esc(next.name)}</strong><p>${esc(next.description||'Проверьте готовность команды и подтверждающих материалов.')}</p>`:'<p>Все этапы завершены.</p>'}</aside>
    </div>
