...
async function installSnapshotPublisher() {
  ...
  try {
    const current = await snapshotContext.supabase.rpc('get_public_project_snapshot', { p_project_code: snapshotContext.projectCode });
    if (!current.error && current.data?.published_at) state.textContent = formatPublishedAt(current.data.published_at);
  } catch (error) { console.warn('[public-snapshot-status]', error); }
  button.addEventListener('click', async () => {
    button.disabled = true;
    button.textContent = 'Публикуем…';
    state.textContent = 'Формируем безопасную открытую версию…';
    try {
      const result = await snapshotContext.supabase.rpc('publish_project_snapshot', { p_project_id: snapshotContext.projectId });
      ...
      state.textContent = text.includes('function') || text.includes('schema cache') ? 'Сначала выполните файл supabase_public_snapshot.sql' : 'Ошибка публикации';
...
function installMobileNavigation() {
  ...
  menuButton.onclick = event => { event.preventDefault(); event.stopPropagation(); setMenuOpen(!sidebar.classList.contains('open')); };
  closeButton.onclick = () => setMenuOpen(false);
  backdrop.onclick = () => setMenuOpen(false);
  nav.addEventListener('click', event => { if (event.target.closest('[data-page]')) setMenuOpen(false); });
  window.addEventListener('hashchange', () => setMenuOpen(false));
  document.addEventListener('keydown', event => { if (event.key === 'Escape') setMenuOpen(false); });
  const handleViewportChange = event => { if (!event.matches) setMenuOpen(false); };
...
function createPasswordModal() {
  ...
  backdrop.innerHTML = `
    <section class="modal" role="dialog" aria-modal="true" aria-labelledby="changePasswordTitle" style="width:min(520px,100%)">
      <div class="modal-head"><div><div class="eyebrow">Безопасность аккаунта</div><h2 id="changePasswordTitle">Сменить пароль</h2></div><button class="icon-button" type="button" data-password-close aria-label="Закрыть">×</button></div>
      <div class="modal-body">
        <form id="changePasswordForm" class="form-grid" style="grid-template-columns:1fr">
          <div class="form-group full"><label>Текущий пароль</label><input name="current_password" type="password" autocomplete="current-password" minlength="8" required></div>
          <div class="form-group full"><label>Новый пароль</label><input name="new_password" type="password" autocomplete="new-password" minlength="8" required></div>
          <div class="form-group full"><label>Повторите новый пароль</label><input name="confirm_password" type="password" autocomplete="new-password" minlength="8" required></div>
        </form>
        <div id="changePasswordMessage" class="auth-message" style="margin-top:12px"></div>
      </div>
      <div class="modal-foot"><button class="btn btn-secondary" type="button" data-password-close>Отмена</button><button class="btn btn-primary" type="submit" form="changePasswordForm" id="changePasswordSubmit">Сохранить новый пароль</button></div>
    </section>`;
  ...
  backdrop.querySelector('#changePasswordForm').addEventListener('submit', async event => {
    ...
    const setMessage = (text, type = '') => { message.textContent = text; message.className = `auth-message ${type}`.trim(); };
    ...
    try {
      ...
      const verify = await snapshotContext.supabase.auth.signInWithPassword({ email, password: currentPassword });
    ...
    } catch (error) { setMessage(String(error.message || 'Не удалось изменить пароль.'), 'error'); }
...
function installPasswordChange() {
  if (!snapshotContext?.supabase || !snapshotContext?.session || document.getElementById('changePasswordButton')) return Boolean(document.getElementById('changePasswordButton'));
  const actions = document.querySelector('.topbar-actions');
  ...
  button.title = 'Изменить пароль аккаунта';
  button.style.whiteSpace = 'nowrap';
  button.addEventListener('click', openPasswordModal);
  const userMenu = actions.querySelector('.user-menu');
  const sync = actions.querySelector('.sync-state');
  if (userMenu) userMenu.insertAdjacentElement('afterend', button);
  else if (sync) actions.insertBefore(button, sync);
  else actions.prepend(button);
  return true;
}

function ensurePasswordChangeVisible() {
  if (installPasswordChange()) return;
  let tries = 0;
  const timer = setInterval(() => {
    tries += 1;
    if (installPasswordChange() || tries >= 20) clearInterval(timer);
  }, 250);
}

setTimeout(installSnapshotPublisher, 250);
setTimeout(installMobileNavigation, 260);
setTimeout(ensurePasswordChangeVisible, 300);
